...
export async function saveSchedule(
    ...
    try {
      ...
      const { availability, ...scheduleData } = data
      ...
      if (availability.length > 0) {
        statements.push(
          db.insert(ScheduleAvailabilityTable).values(
            availability.map(availabilities => ({
              ...availabilities,
  ...
  // Keep only the time slots that satisfy two conditions:
    ...
    availabilities.some(availabilities => {
      return (
        isWithinInterval(eventInterval.start, availabilities) && // Start is inside availability
        isWithinInterval(eventInterval.end, availabilities) // End is inside availability
      )
    })
  )
